...
.extract_random_variances.default <- function(
  model,
  ci = 0.95,
  effects = "random",
  ...
  out <- suppressWarnings(.extract_random_variances_helper(
    model,
    ci = ci,
    effects = effects,
    component = component,
...
.extract_random_variances.glmmTMB <- function(
  model,
  ci = 0.95,
  effects = "random",
  component = "all",
  ...
  if (
    insight::model_info(model, verbose = FALSE)$is_zero_inflated &&
      !is.null(insight::find_random(model)$zero_inflated_random)
  ) {
    # nolint
    zi_var <- suppressWarnings(.extract_random_variances_helper(
      model,
      ci = ci,
      effects = effects,
      component = "zi",
...
.extract_random_variances_helper <- function(
  model,
  ci = 0.95,
  effects = "random",
  component = "conditional",
  ...
  re_sd_intercept <- re_data$var1 == "(Intercept)" &
    is.na(re_data$var2) &
    re_data$grp != "Residual"
  re_sd_slope <- re_data$var1 != "(Intercept)" &
    is.na(re_data$var2) &
    re_data$grp != "Residual"
  re_cor_intercept <- re_data$var1 == "(Intercept)" &
    !is.na(re_data$var2) &
    re_data$grp != "Residual"
  re_cor_slope <- re_data$var1 != "(Intercept)" &
    !is.na(re_data$var2) &
    re_data$grp != "Residual"
  re_sigma <- re_data$grp == "Residual"

  # for special covariance structures, we now update the rows we want to keep
  ...
  pattern <- paste0(
    "(",
    paste(insight::find_random(model, flatten = TRUE), collapse = "|"),
    ")\\.\\d+$"
  )
...
# this is a small helper function to update the row indices to select
# for special covariance structures. E.g., for "ar1", we don't want to
...
.update_indicator_rows <- function(indicator_rows) {
  if (any(indicator_rows)) {
    ...
    # by default - all rows that should be selected are "TRUE". For certain
    # cov-structures, we just want to keep the first "TRUE" value and set all
    # remaining elements to FALSE.
    trues <- which(indicator_rows)
    indicator_rows[indicator_rows] <- FALSE
    indicator_rows[trues[1]] <- TRUE
  }
  indicator_rows
}
...
as.data.frame.VarCorr.coxme <- function(x, row.names = NULL, optional = FALSE, ...) {
  ...
  out <- do.call(
    rbind,
    lapply(names(variances), function(i) {
...
.random_sd_ci <- function(
  ...
  if (inherits(model, c("merMod", "glmerMod", "lmerMod"))) {
    ...
    if (!is.null(ci_method) && ci_method %in% c("profile", "boot")) {
      out <- tryCatch(
        {
          ...
          out$CI_low[!corr_param & !sigma_param] <- var_ci$CI_low[
            !var_ci_corr_param & !var_ci_sigma_param
          ]
          out$CI_high[!corr_param & !sigma_param] <- var_ci$CI_high[
            !var_ci_corr_param & !var_ci_sigma_param
          ]
      ...
      if (
        all(suppressMessages(insight::check_if_installed(
          c("merDeriv", "lme4"),
          quietly = TRUE
        )))
      ) {
        ...
        out <- tryCatch(
          {
            ...
            if (any(rnd_slope_corr)) {
              for (gr in setdiff(unique(out$Group), "Residual")) {
                ...
                dummy <- gsub(
                  "Cor \\((.*)~(.*)\\)",
                  "\\2.\\1",
                  out$Parameter[rnd_slope_corr_grp]
                )
                var_ci$Parameter[var_ci$Group == gr][match(
                  dummy,
                  var_ci$Parameter[var_ci$Group == gr]
                )] <- out$Parameter[rnd_slope_corr_grp] # nolint
              }
            }

            # remaining
            var_ci_others <- !grepl("^(Cor|SD) (.*)", var_ci$Parameter)
            var_ci$Parameter[var_ci_others] <- gsub(
              "(.*)",
              "SD (\\1)",
              var_ci$Parameter[var_ci_others]
            )
            ...
            if (any(var_ci_corr_param)) {
              ...
              out$CI_low[var_ci_corr_param] <- tanh(
                atanh(coefs) - stats::qnorm(0.975) * delta_se
              )
              out$CI_high[var_ci_corr_param] <- tanh(
                atanh(coefs) + stats::qnorm(0.975) * delta_se
              )
            }
            ...
            out$CI_low[!var_ci_corr_param] <- exp(
              log(coefs) - stats::qnorm(0.975) * delta_se
            )
            out$CI_high[!var_ci_corr_param] <- exp(
              log(coefs) + stats::qnorm(0.975) * delta_se
            )
            ...
            if (
              isTRUE(verbose) &&
                insight::check_if_installed("performance", quietly = TRUE) &&
                isTRUE(performance::check_singularity(model))
            ) {
              # nolint
              insight::format_alert(
          ...
          error = function(e) {
            if (isTRUE(verbose)) {
              if (grepl("nAGQ of at least 1 is required", e$message, fixed = TRUE)) {
                insight::format_alert(
                  "Argument `nAGQ` needs to be larger than 0 to compute confidence intervals for random effect parameters."
                ) # nolint
              }
              if (
                grepl("Multiple cluster variables detected.", e$message, fixed = TRUE)
              ) {
                insight::format_alert(
                  "Confidence intervals for random effect parameters are currently not supported for multiple grouping variables."
                ) # nolint
              }
              if (
                grepl("exactly singular", e$message, fixed = TRUE) ||
        ...
        insight::format_alert(
          "Package 'merDeriv' needs to be installed to compute confidence intervals for random effect parameters."
        ) # nolint
      }
    }
    ...
    out <- tryCatch(
      {
        ...
        if (isTRUE(verbose)) {
          ...
          singular_fit <- insight::check_if_installed("performance", quietly = TRUE) &
            isTRUE(performance::check_singularity(model)) # nolint
...
.unregister_vcov <- function() {
  ...
  suppressWarnings(suppressMessages(registerS3method(
    "vcov",
    "lmerMod",
    method = lme4::vcov.merMod
  )))
  suppressWarnings(suppressMessages(registerS3method(
    "vcov",
    "glmerMod",
