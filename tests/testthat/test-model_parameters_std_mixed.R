if (require("testthat") &&
  ...
  test_that("model_parameters, standardize-refit, wald-normal", {
    skip_on_cran()
    params <- model_parameters(model, ci_method = "normal", standardize = "refit", verbose = FALSE, effects = "fixed")
  ...
  test_that("model_parameters, standardize-posthoc", {
    skip_on_cran()
    params <- model_parameters(model, ci_method = "normal", standardize = "posthoc", verbose = FALSE, effects = "fixed")
    expect_equal(c(nrow(params), ncol(params)), c(7, 10))
    expect_equal(params$Std_Coefficient, c(0, 0.49679, -0.49355, 0.34791, 1.74252, -0.25421, -0.18834), tolerance = 1e-3)
    expect_equal(params$SE, c(0, 0.66228, 0.70202, 0.05968, 0.13914, 0.09762, 0.0945), tolerance = 1e-3)
  ...
  test_that("model_parameters, standardize-basic", {
    skip_on_cran()
    params <- model_parameters(model, ci_method = "normal", standardize = "basic", verbose = FALSE, effects = "fixed")
    expect_equal(c(nrow(params), ncol(params)), c(7, 10))
