if (require("testthat") && require("parameters") && require("effectsize") && require("lme4")) {
  ...
  testthat::expect_true(all(names(t) == c("Parameter","Coefficient","SE","95% CI","t(147)","p")))

  x <- parameters::model_parameters(lme4::lmer(Sepal.Length ~ Petal.Length + (1|Species), data=iris))
  t <- parameters::parameters_table(x)
  testthat::expect_true(all(names(t) == c("Parameter","Coefficient","SE","95% CI","t(146)","p")))
  ...
  t <- parameters::parameters_table(x)
  testthat::expect_true(all(names(t) == c("Parameter", "Std. Coef.", "SE", "95% CI", "t(147)", "p")))
